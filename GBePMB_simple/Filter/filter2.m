function [d_gospa,decomposed_cost,card_est,lambdaC,Pd_hat,est,est_gt,runtime,GOSPA_] = filter2(birthmodel,measmodel,motionmodel,ppp,mbm,paras,groundTruth,Z,model,groundTruth_target_irregular,T)
%
gt = groundTruth.gt;

card = groundTruth.card;
plot_enable = 0;
...
for t = 1:T
    ...
    if paras.mb_approx && n_mb > 1
        ...
        for i = 1:n_track_t
            ...
            if paras.Pd_est
                [mb_approx(i).s,mb_approx(i).t] = ...
                    beta_merge_nobias(mbm_upd.track{i},w_margin_n);
            else
                mb_approx(i).s = paras.Pd_hat*10;
                mb_approx(i).t = (1-paras.Pd_hat)*10;
            end
            mb_approx(i).label = mbm_upd.track{i}(1).label;
            mb_approx(i).labellast = mbm_upd.track{i}(1).labellast;
        end
        ...
        elseif paras.mb_merge == 3 && n_track_t > 1
            ...
            bern0 = struct('r',0,'xr',zeros(dxr,1),...
                'Cr',ones(dxr,dxr),'V',zeros(2,2),'v',0,...
                'alpha',1,'beta',1,'isDetected',false,'s',paras.Pd_hat*10,'t',(1-paras.Pd_hat)*10);
        ...
        if paras.recycle && (~paras.mb_birth)
            ...
            for j = 1:length(idx)
                ...
                ppp(end,1).label = mbm_upd.track{idx(j)}.label;
                ppp(end,1).labellast = mbm_upd.track{idx(j)}.labellast;
            end
        end
        ...
        for i = 1:length(mbm_upd.track)
            ...
            for j = 1:length(idx)
                ...
                if paras.recycle && (~paras.mb_birth)
                    ...
                    ppp(end,1).label = mbm_upd.track{i}(idx(j)).label;
                    ppp(end,1).labellast = mbm_upd.track{i}(idx(j)).labellast;
                end
            end
            %delete these single target hypotheses
            %re-index
    ...
    if paras.clutter_est
        %预测步
        eta = 1.05;%遗忘因子
        paras.clutter.alpha = paras.clutter.alpha/eta;
        paras.clutter.beta = paras.clutter.beta/eta;
        %更新步
        ...
        lambdaC(t) = paras.clutter.lambdaC_hat;
        Nc_hat = [Nc_hat;nc_hat];
    else
        % 若不适应估计器，则一直保持初始值。
        paras.clutter.lambdaC_hat = paras.clutter.lambdaC_hat;
        paras.clutter.c_intensity_hat = paras.clutter.c_intensity_hat;
    end
    %% ppp修剪
    dxr = motionmodel.dxr;
    if ~paras.mb_birth
        ...
        % idx = ([ppp.w] >  log(1e-5)) & ([ppp.alpha]./[ppp.beta] > 1) & ([ppp.v] > 7);
        ppp = ppp(idx);
        if ~paras.mb_approx
            ...
            for i = 1:length(hypo_best)
                if mbm_upd.table(I,i) > 0
                    ...
                    if mbm_upd.track{i}(mbm_upd.table(I,i)).r > paras.estimate.r
                        ...
                        est{t}.pd = [est{t}.pd [mbm_upd.track{i}(mbm_upd.table(I,i)).s;...
                            mbm_upd.track{i}(mbm_upd.table(I,i)).t;...
                            pd_hat;...
                            mbm_upd.track{i}(mbm_upd.table(I,i)).isDetected;...
                            mbm_upd.track{i}(mbm_upd.table(I,i)).r]];
                        % est{t}.r = [est{t}.r mbm_upd.track{i}(mbm_upd.table(I,i)).r];
                        est{t}.label = [est{t}.label [mbm_upd.track{i}(mbm_upd.table(I,i)).label;mbm_upd.track{i}(mbm_upd.table(I,i)).labellast]];
...
%% 检测概率估计器
    if ~isempty(mbm_upd.table)
        if est{t}.card>0
            if paras.Pd_estimator == 1
                est{t}.pd_total = mean(est{t}.pd(3,:),2);
                est{t}.pd(6,:) = 1/length(est{t}.pd(3,:));                
            else
                factor_w = 1;%方差权重系数
                %计算方差
                pd_var = est{t}.pd(1,:).*est{t}.pd(2,:)./((est{t}.pd(1,:)+est{t}.pd(2,:)).^2)./(est{t}.pd(1,:)+est{t}.pd(2,:)+1);
                %计算权重
                w_var = (1./(pd_var.^factor_w))./sum(1./(pd_var.^factor_w));
                est{t}.pd_total = w_var*est{t}.pd(3,:)';
                est{t}.pd(6,:) = w_var;

                % 解决检查概率与量测率之间的耦合
                p_pois0 = exp(-est{t}.x_lambda);
            end
        else %仍存在Bernoulli中，单r小于阈值
            if paras.Pd_estimator == 1
                est{t}.pd_total = (exp([ppp.w])/sum(exp([ppp.w])))*([ppp.s]./([ppp.s]+[ppp.t]))';
                est{t}.pd_total = paras.Pd_hat;
    ...
    if plot_enable
        ...
        H = [H h_Z];
        [~,I] = max(mbm_upd.w);%find the global hypothesis with the highest weight
        if ~isempty(mbm_upd.table)
            hypo_best = mbm_upd.table(I,:);
            for i = 1:length(hypo_best)
                if mbm_upd.table(I,i) > 0
    ...
    [estimates_ks{t},~] = estimator(mbm_upd,model);
    GOSPA_(t,:) = GOSPAmetric_EDM(estimates_ks{t},groundTruth_target_irregular{t},20,1);
    ...
    mbm.track = cellfun(@(x) arrayfun(@(x) ...
        BGGbern_pred(x,motionmodel,paras),x),mbm_upd.track,'uniformoutput',false);
    ...
    if paras.mb_birth
        ...
        ppp = BGGppp_pred(ppp,motionmodel,birthmodel,paras);
    end
%     [estimates_{k},trajectoryEstimates_{t}{k}] = estimator(MBM,model);
    %% 
...
if  0
        figure(1);
         if ~isempty(Z{t})
            plot(Z{t}(1,:),Z{t}(2,:),"k.",'HandleVisibility','off');
         end

         estimates_improved = estimates_ks;
        i0=size(estimates_improved{t}.g,2);
        for j = 1:i0
            figure(1);
            hold on
            [x, y] = Sigmacircle(estimates_improved{t}.x(1,j),estimates_improved{t}.x(2,j),estimates_improved{t}.X(:,:,j),2,j);
...
runtime = sum(t_elapsed);

est_gt =  repmat(struct('label',-1,'labellast',-1,'x_sur',[],'x_lambda',[],'pd_hat',0,'beta',[],'xr',[],'X',[]),5000,1);
nj = 0;
for t=1:T
    for i = 1:length(est{t}.label(1,:))
        idx = est{t}.label(1,i);
        est_gt(idx).label = idx;
        % est_gt(idx).r = [est_gt(idx).r est{t}.r(i)];
        ...
        est_gt(idx).labellast = est{t}.label(2,i);
    end
end
