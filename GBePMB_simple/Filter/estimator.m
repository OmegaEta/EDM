...
estimates.r = cell(0,1);
estimates.shape = zeros(2,2,model.direction,0);
d = 2;
...
%             estimates.g = [estimates.g MBM.track{i}(table_entry(i)).Bern.GGIW(end).a/MBM.track{i}(table_entry(i)).Bern.GGIW(end).b];
            estimates.x = [estimates.x MBM.track{i,1}.xr];
            X = MBM.track{i,1}.V/(MBM.track{i,1}.v-2*d-2);
            estimates.X = cat(3,estimates.X,X);
            Shape_coefficients = Sigmacircle_Eplision(MBM.track{i,1}.xr(1),...
                MBM.track{i,1}.xr(2),X,2,model);
            estimates.r = cat(1,estimates.r,Shape_coefficients.r);
            estimates.shape = cat(4,estimates.shape,Shape_coefficients.shape);
%         end
