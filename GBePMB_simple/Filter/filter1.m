function [d_gospa,decomposed_cost,card_est,lambdaC,est] = filter1(birthmodel,measmodel,motionmodel,ppp,mbm,paras,groundTruth,Z)
%
gt = groundTruth.gt;
gt_time = groundTruth.gt_time;
...
for t = 1:T
    ...
    bern_new = repmat(struct('r',0,'xr',zeros(dxr,1),'Cr',ones(dxr,dxr),...
        'V',zeros(2,2),'v',0,'alpha',1,'beta',1,'isDetected',false),1,n_clusters);
    ...
    for i = 1:n_track
        ...
        for j = 1:n_local_hypo(i)
            ...
            [l_missed,bern_missed] = BGGbern_miss(mbm.track{i}(j),measmodel);
            tracks_upd{i}{j}.c = 0;
            ...
            for c = 1:n_clusters
                ...
                if sum(gating_matrix_d{i}(:,j)-clusters(:,c)<0) == 0
                    [l_upd,bern_updated] = ...
                        BGGbern_upd(mbm.track{i}(j),W(:,clusters(:,c)),measmodel);
    ...
    if (paras.new_track_init == 2) && paras.mb_approx || paras.mb_birth
        ...
        for i = 1:nm
            ...
            tracks_new{i}.bern = struct('r',0,'xr',zeros(dxr,1),'Cr',...
                ones(dxr,dxr),'V',zeros(2,2),'v',0,'alpha',1,'beta',1,"isDetected",false);
        end
        for c = 1:n_clusters
    ...
    if paras.mb_approx && n_mb > 1
        ...
        elseif paras.mb_merge == 3 && n_track_t > 1
            ...
            bern0 = struct('r',0,'xr',zeros(dxr,1),...
                'Cr',ones(dxr,dxr),'V',zeros(2,2),'v',0,...
                'alpha',1,'beta',1,'isDetected',false);
...
% clf(7);
% hold on;
% plot(1:T,ones(T,1)*measmodel.c_lambda,"k");
% plot(1:T,lambdaC,"r","LineWidth",2);
% plot(1:T,Nc_hat,"b");
% 
fprintf('Mean GOSPA: %.2f\n',mean(d_gospa))
% fprintf('Total runtime: %.2f\n',sum(t_elapsed))
fprintf('mean Nc_hat: %.2f\n',mean(Nc_hat))
end
