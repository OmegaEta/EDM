function plotTrajectory(gt,motionmodel,figid)
% 或者轨迹，并每五个时刻绘制一次目标扩展
% @gt - 目标集合
% @figid - 绘图隔窗编号
% @return
...
for i = 1:nt
    %plot trajectory
    TH= plot(gt(i).xr(1,:),gt(i).xr(2,:),typeset(i),'linewidth',1,'DisplayName',strcat('target',num2str(i)));
...
legend([trajectory_handle gt_es_plot start_handle end_handle], ...
    {'Target1','Target2','Target3','Target4','Target extent','Birth position','Death position'});
end
