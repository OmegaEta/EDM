...
for i = 1:n_tt
    ...
    for j = 1:num_hypo
        ...
        if MBM.track{i}(j).Bern.t_death(end) == time
            gating_matrix_d{i}(:,j) = irregularGating(W,MBM.track{i}(j).Bern.GGIW(end),model);
        end
    end
    ...
    for j = 1:length(meas_track{i})
        for k = 1:size(meas_track{i}{j},1)
            ...
            if any(meas_track{i}{j}(k,:))
                [Bern,lik] = detectionBernforUpdate(MBM.track{i}(j).Bern,W(:,meas_track{i}{j}(k,:)),model);
            else
...
if n_tt_upd > n_tt
    for i = 1:n_tt_upd - n_tt
        ...
        if any(in_gate)
            [Bern,lik] = detectionPPPforUpdate(PPP.w(in_gate),PPP.GGIW(in_gate),W(:,meas_track{n_tt+i}{1}),model);
            Bern.t_birth = time;
            Bern.t_death = time;
            Bern.w_death = 1;
        else
...
% %Shape penalty function.
% reduce_shape(tracks,W,model);

%UPDATE EDM
tracks_ = ExtensionDifferenceFunc(tracks,W,model);
tracks = tracks_;
