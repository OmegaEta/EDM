function [ Error ] = GOSPAmetric_EDM(X,Y,c,p)
...
% X.x -- Nx * J matrix with kinematic vectors     理解为运动信息
% X.X -- d * d * J tensor with random matrices    理解为形状信息
...
if Nx==0 && Ny==0
    ...
    FalseError = 0;
    shape_error = 0;
    Error = [gospa,shape_error,LocationError,MissedError,FalseError];
    return
elseif Nx==0
    ...
    FalseError = 0;
    shape_error = 0;
    Error = [gospa,LocationError,shape_error,MissedError,FalseError];
    return
elseif Ny==0
    ...
    FalseError = Nx;
    shape_error = 0;
    Error = [gospa,LocationError,MissedError,FalseError,shape_error];
    return
end

% Distance matrix
D = repmat(c,[Nx Ny]);
% D_gsd = repmat(c,[Nx Ny]);
D_Dr=repmat(c,[Nx Ny]);
for ix = 1:Nx
    for iy = 1:Ny
        ...
        gsd = computeRadialRMSE(X.x(1:2,ix),Y.x(1:2,iy),X.r{ix},Y.r{iy},X.shape,Y.shape);
...
if Ny<=Nx
    ...
    for iy = 1:Ny
        ...
        if D(Customer2Item(iy),iy) < c
            ...
            absGamma = absGamma + 1;
            % shape error
            shapeError = shapeError + D_Dr(Customer2Item(iy),iy);
        end
    end
else
    % Compute assignment
    ...
    for ix = 1:Nx
        ...
        if D(ix,Customer2Item(ix)) < c
            ...
            absGamma = absGamma + 1;
            % shape error
            shapeError = shapeError + D_Dr(ix,Customer2Item(ix));
        end
    end
end

gospa = ((gospa + 0.5*(c^p)*(Nx+Ny-2*absGamma))^(1/p))/2;
% Missed detection error
MissedError = Ny-absGamma;

if Ny~=Nx
    shapeError = shapeError * (Ny/Nx);
end

% False alarm error
FalseError = Nx-absGamma;

Error = [gospa,LocationError,shapeError,MissedError,FalseError];
