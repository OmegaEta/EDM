...
NUM_A=zeros(T,N);
for n = 1:N
    filename1 = strcat('./MBM/平行场景轨迹/trajectoryEstimates_improved',string(s),'_',string(n),'.mat');
    filename2 = strcat('./MBM/平行场景轨迹/trajectoryEstimates_original',string(s),'_',string(n),'.mat');
    load(filename1)
    load(filename2);
    for i=1:100
        NUM_A(i,n)=2;
