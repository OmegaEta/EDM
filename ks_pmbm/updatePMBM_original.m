function [PPP,MBM,meas_cell00] = updatePMBM_original(PPP,MBM,W,time,model)
...
if ~any(idx_keep)
    [~,idi] = sort(wAssoc,'descend');
    if ~isempty(idi)
       idx_keep(idi(1:min(length(idi),10))) = true;
    end
end
% for i=1:7
%     if(sum(idx_keep))>20
%         idx_keep = wAssoc > log(model.threshold_w*(i+1));
%     else
%         break;
%     end
% end
wAssoc = wAssoc(idx_keep);
if length(wAssoc) == 1; wAssoc = 0; end%直接决定当前时刻的全局假设数
...
for i = 1:n_tt
    idx = 0;
    for j = 1:length(meas_track{i})
        for k = 1:size(meas_track{i}{j},1)%上一个时刻的用一条track分支，用不同的量测来更新
            idx = idx + 1;
...
j = 0;
idi_meas = table(idi,:);
idi_track = find(idi_meas>0);
len = size(idi_track,2);
meas_cell00 = cell(len,1);
for i = idi_track
    N_meas = 0;
    temp1 = 0;
    temp2 = 0;
    if isempty(meas_track) || i>size(meas_track,1)|| isempty(meas_track{i})
        continue;
    end
    for k = 1:size(meas_track{i},1)
        N_temp = size(meas_track{i}{k},1);
        N_meas = N_meas + N_temp;
        if idi_meas(i)<=N_meas
            temp1 = k;
            temp2 = idi_meas(i) - (N_meas-N_temp);
            break;
        end
    end
    if temp2 == 0 || temp1 == 0
    ...
    j = j+1;
    meas_cell00{j} = W(:,meas_track{i}{temp1}(temp2,:))';
end
